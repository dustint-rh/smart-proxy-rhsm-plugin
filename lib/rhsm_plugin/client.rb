...
module RhsmProxy
  class ClientBase
    ...
    def self.http
      uri = URI.parse(::RhsmProxy::Plugin.settings.rhsm_url.to_s)
  ...
  class RhsmClient < ClientBase
    def self.get(path)
      super(::RhsmProxy::Plugin.settings.rhsm_url.to_s, path)
    end
  end
end
